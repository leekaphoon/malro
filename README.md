...
| AI | **각자 자기 Gemini 키**(BYO) — 설정에 키만 붙여넣음 | 서버 배포 없음. 이유는 바로 아래 |
...
그래서 UX 는 이렇다: **Google 로그인만 하면 태스크 앱으로 완전히 동작하고, AI 는 설정에
Gemini API 키만 붙여넣으면 켜진다.** 브라우저가 Gemini 를 직접 호출하고(키는 이 브라우저에만
저장), 할 일 변경은 이미 로그인된 권한으로 처리하므로 **사용자가 세울 서버가 없다.**

> **시리 음성은 예외다.** iOS 단축어는 Google 로그인을 못 하므로 브라우저 토큰을 쓸 수 없다.
> 그래서 시리를 쓰려는 사람만 별도로 자기 Apps Script 를 배포하고 단축어를 만든다
> (`voice/SHORTCUT.md`). 이건 설정에 키를 넣는 것으로는 안 되는, 시리 고유의 한 단계다.
...
이 앱은 **서버가 없다.** 정적 파일만 올리면 되고, 데이터는 각 사용자의 Google Tasks 에만
있다. 그래서 포크해서 자기 것으로 돌리기가 어렵지 않다. **저장소에 키는 하나도 없다**
— 배포 스크립트가 키 형태의 문자열을 발견하면 배포를 멈춘다(`SECURITY.md`).

**1. 포크 / 클론**

```bash
git clone https://github.com/<your-name>/malro.git && cd malro
```

**2. Google OAuth 클라이언트 발급** — 아래 「OAuth 클라이언트 발급 절차」 그대로. 약 5분.
발급받은 **클라이언트 ID** 하나가 필요하다.

**3. 클라이언트 ID 를 넣는다** — 세 가지 방법 중 하나:

- `app.js` 의 `DEFAULT_CLIENT_ID` 를 네 ID 로 교체 (포크 배포용 — 권장)
- 배포한 앱의 설정 → **클라이언트 ID 변경** 에 입력 (코드 안 고치고 이 기기만)
- 주소 뒤에 `?cid=<네 클라이언트 ID>` (임시 확인용)

**4. 정적 배포** — 서버가 없으니 어디든 된다.

- **GitHub Pages**: 저장소 Settings → Pages → 무설정으로 바로. (앱 파일이 루트에 있어 그대로 서빙된다)
- **Cloudflare Workers/Pages**: GitHub 연동. `wrangler.jsonc` 가 `build.sh` 로 앱 파일 11개만 골라 `dist/` 를 서빙한다. push 하면 자동 배포
- **로컬 확인**: `python3 -m http.server 4173` 로 열어도 된다 (`http://localhost:4173`)

**5. 배포 주소를 OAuth 승인된 JavaScript 원본에 추가** — 오리진만, 끝 슬래시·경로 없이
(예: `https://<네-주소>`). 반영에 몇 분 걸린다.

여기까지면 **할 일 앱**이 네 계정으로 완전히 돈다. 아래 둘은 선택이다.

- **AI 어시스턴트** — 설정(톱니) 에 **Gemini API 키만 붙여넣으면** 켜진다. 서버 배포 없음.
  키는 [AI Studio](https://aistudio.google.com) 에서 무료로 발급
- **시리 음성** — `voice/SHORTCUT.md`

> **혼자·지인끼리 쓰면 이걸로 끝이다.** 각자 자기 프로젝트를 쓰므로 아래 100명 상한도
> 각자 것이라 신경 쓸 일이 없다.
>
> ⚠ **모르는 사람까지 네 인스턴스에 로그인시키려면** OAuth 검증이 필요하다. `auth/tasks`
> 는 민감 범위라, 미검증 상태에서는 **누적 100명**(동시 아님, 프로젝트 수명 전체, 초기화
> 불가) 상한과 "확인되지 않은 앱" 경고 화면이 걸린다. 검증은 개인정보처리방침·도메인
> 소유 확인·데모 영상, 심사 약 10영업일, 무료. **공개 글로 홍보하기 전에 미리 신청**해
> 두는 편이 낫다. (출처: Google OAuth 검증 FAQ, 2026-09 확인)
